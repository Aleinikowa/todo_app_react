...
function formatDateForPanel(prefix, date) {
  ...
  const timePart = `${date.getHours()}:${(minutes ? (minutes > 9 ? minutes : '0' + minutes) : '00')}`;
...
function renderCreatedDatePanel(doc, todo) {
  ...
  createdDatePanel.innerHTML = formatDateForPanel("<span>Created</span>", todo.dateCreated);
...
function renderDoneDatePanel(doc, todo) {
  ...
  doneDatePanel.innerHTML = formatDateForPanel("<span>Completed</span>", todo.dateCompleted);
